...
public class DatabaseHelper {
	...
	private void createTables() throws SQLException {
	    // First check if the oneTimePassword column exists
	    boolean columnExists = false;
	    try {
	        ResultSet rs = connection.getMetaData().getColumns(null, null, "CSE360USERS", "ONETIMEPASSWORD");
	    ...
	    // Add the oneTimePassword column if it doesn't exist
	    if (!columnExists) {
	        try {
	            String alterTable = "ALTER TABLE cse360users ADD COLUMN oneTimePassword VARCHAR(255)";
	            statement.execute(alterTable);
	        } catch (SQLException e) {
	            // Column might have been added by another connection
	            System.out.println("Note: oneTimePassword column may already exist");
	        }
	    }
