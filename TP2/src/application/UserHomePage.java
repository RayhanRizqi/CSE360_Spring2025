...
public class UserHomePage {
    ...
    public void show(Stage primaryStage) {
	    ...
	    // Label to display Ask a question
	    Label questionLabel = new Label("Ask a Question!");
	    questionLabel.setStyle("-fx-font-size: 16px;");
	    
	    // Textfield for the question
	    TextField questionText = new TextField();
	    questionText.setPromptText("Enter Question");
	    questionText.setMaxWidth(250);
	    
	    // Submit button for question
	    Button submissionButton = new Button("Submit");
	    
	    submissionButton.setOnAction(a -> {
	    	String question = questionText.getText();
	    	Question newQuestion = new Question(question, user.getUserName());
	    	databaseHelper.addQuestion(newQuestion);
	    });
	    
	    HBox questionBox = new HBox();
	    questionBox.setStyle("-fx-alignment: center; -fx-padding: 20;");
	    questionBox.getChildren().addAll(questionLabel, questionText, submissionButton);
	    ...
	    viewQuestionsButton.setOnAction(a -> {
	    	new ViewQuestionsPage(user, databaseHelper).show(primaryStage);
	    });

	    layout.getChildren().addAll(userLabel, logOutButton, questionBox, viewQuestionsButton);
